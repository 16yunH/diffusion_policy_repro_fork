...
RUN_DIR="${RUN_DIR:-}"
SESSION_NAME="${SESSION_NAME:-dp_pusht_single_s${SEED}}"
...
resolve_run_dir() {
  ...
  printf 'data/outputs/pusht_image_single_seed_%s_%s\n' "${SEED}" "$(date +%Y%m%d_%H%M%S)"
}
...
PY
  } > "${out_dir}/single_seed_env_info.txt"
}
...
main() {
  ...
  if command -v tmux >/dev/null 2>&1; then
    ...
    echo "tmux: ${SESSION_NAME}" > "${run_dir}/single_seed_launcher.txt"
    log "Attach with: tmux attach -t ${SESSION_NAME}"
  else
    nohup bash -lc "cd '${REPO_ROOT}' && ${cmd}" > "${run_dir}/single_seed_nohup.log" 2>&1 &
    echo "pid: $!" > "${run_dir}/single_seed_launcher.txt"
